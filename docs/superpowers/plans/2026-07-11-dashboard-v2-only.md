...
**Files:** `models/dashboard_v2_base.py`、`models/dashboard_v2.py`、`services/dashboard_trigger.py`、`backend/routers/status.py`、`tests/test_dashboard_trigger.py`、`tests/test_dashboard_v2_run_store.py`。

- [ ] 在 `tests/test_dashboard_trigger.py` 新增 `test_load_dashboard_config_rejects_non_v2_schema`：写入 `{"schema_version": 1}`，断言 `load_dashboard_config()` 抛出匹配“只支持.*2”的 `ValueError`。
- [ ] 运行 `$env:PYTHONPATH='.'; pytest tests/test_dashboard_trigger.py -q`，确认新测试因当前配置允许 V1 而失败。
...
- [ ] 将 `services/dashboard_trigger.py` 和 `backend/routers/status.py` 的运行记录读写改为 `infrastructure.dashboard_v2_run_store.MySQLV2CollectionRunStore`，不再导入 V1 store。
- [ ] 运行 `$env:PYTHONPATH='.'; pytest tests/test_dashboard_trigger.py tests/test_dashboard_v2_run_store.py -q`，预期通过；提交 `refactor: isolate dashboard v2 runtime dependencies`。

### Task 2: 固定 V2 任务、健康检查与运维入口

**Files:** `tasks/dashboard_tasks.py`、`backend/services/health_service.py`、`scripts/lib/prefect_start.ps1`、`scripts/tools/dashboard/run_collection.py`、`tests/test_dashboard_tasks.py`、`tests/test_health_service.py`。

- [ ] 在 `tests/test_dashboard_tasks.py` 新增 V1 配置调用 `run_dashboard_metric_task.fn()` 抛出 V2-only 配置错误的测试，并先运行确认失败。
...
- [ ] 运行 `$env:PYTHONPATH='.'; pytest tests/test_dashboard_tasks.py tests/test_health_service.py -q`，预期通过；提交 `refactor: make dashboard execution v2 only`。

### Task 3: 删除 V1 源码、迁移和测试

**Files:** 删除 `alembic_dashboard.ini`、`migrations/dashboard/`、`models/dashboard_base.py`、所有不含 `_v2_` 的 `models/dashboard_*.py`、`infrastructure/dashboard_run_store.py`、所有不含 `_v2_` 的 V1 `services/dashboard_*.py` 与对应 `tests/test_dashboard_*.py`。
