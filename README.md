...
源指标的“独立展示”“结果落库/公式组件”和“被公式引用”是三个独立维度：`enabled` 控制是否作为独立指标展示，`storage_mode` 控制结果是否直接落库，公式引用由 `indicator_formula_component` 关系确定。公式编辑器只保存源指标和系数，不再在公式行中修改源指标的全局 `storage_mode`；存储角色必须在源指标页统一设置。

自定义指标保存时会展示公式预览和每个依赖的状态。已归档源指标不能再被新公式引用；已独立停用的源指标仍可作为公式组件使用，因此页面会明确标记“独立停用”，不能把它理解为公式依赖失效。顶部“被公式引用”筛选是关系筛选，与“独立展示”“结果落库”可以重叠，统计数量不能相加。
...
云电脑使用 FRP 部署时，可将配置中心的本地 `5173` 映射为远端 `15173`，将数据驾驶舱的本地 `5174` 映射为远端 `15176`。对应的公网访问地址为 `http://服务器地址:15173/` 与 `http://服务器地址:15176/`。
