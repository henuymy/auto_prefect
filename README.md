...
| 当月累计/累计日期查询（`/api/dashboard/acc`） | 已发布考核版本 | 累计值来自历史累计数据，目标按累计日期匹配 `DAY` 考核版本。 |

实时查询找不到命中的实时草稿时，为兼容已有环境会回退到已发布考核版本。因此完成迁移后应尽快选择一份已填好目标值、且生效区间覆盖当前业务日期的草稿作为实时目标；不能把这个回退当作常规配置方式。
...
发布空草稿会被拒绝。实时草稿则应先保存完整目标值再选择：空草稿虽然不能发布，却会让匹配它的实时页面没有可计算的目标。设置生效日在未来的草稿为实时目标也会取消原草稿的实时资格；在未来日期到来前，当前实时查询会回退到考核版本。因此，切换实时目标前必须同时检查目标值条数和生效日期。
