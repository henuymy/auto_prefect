...
- `city_ops` 探活使用连接超时 `2` 秒、读取超时 `3` 秒和重试间隔 `2` 秒。仅 `requests` 网络异常会重试一次，总共最多两次；已获得的 HTTP 响应不重试，`302`、`401`、`403` 仍按认证失效处理。两次网络异常后仅记录安全的异常类别，例如 `ConnectTimeout` 或 `ReadTimeout`。
