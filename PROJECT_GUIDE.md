...
- 云电脑使用 FRP 时，当前 `frp/frpc.toml` 使用 `5173 -> 15173` 和 `5174 -> 15174`；外部访问地址分别为 `http://服务器地址:15173/` 与 `http://服务器地址:15174/`。只有在同步修改 FRP 配置并重启 `frpc` 后，才能使用其他远端端口。
- 驾驶舱历史挡位调用同源 `/api/dashboard/history/...`，由 `5174` 的 Vite `/api` 代理转发到本机后端 `8000`，不需要把 `8000` 单独暴露到公网。若本地历史请求正常而外部失败，先核对外部端口是否为 `15174`，再检查 FRP 隧道和服务器安全组。
...
- 修改内容：启动、状态和停止脚本分别管理配置中心 `5173` 与驾驶舱 `5174`；驾驶舱 Vite 模式将根路径映射到驾驶舱页面，配置中心拒绝 `/dashboard.html`，驾驶舱拒绝 `/index.html`。当前 FRP 映射为 `15173`、`15174`。
- 配置或迁移：云电脑 FRP 配置应将 `localPort = 5173` 映射到 `remotePort = 15173`，将 `localPort = 5174` 映射到 `remotePort = 15174`；无需数据库迁移。
