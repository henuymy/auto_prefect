"""Commit an updated temporary template as the official template."""
...
def run(config_path=None):
    ...
    update_manifest_path = resolve_path(config_dir, config.get("update_manifest_path"))
    send_result_path = resolve_path(config_dir, config.get("send_result_path"))
    backup_dir = resolve_path(config_dir, config.get("backup_dir", "runtime/backups"))
    manifest_path = resolve_path(config_dir, config.get("manifest_path", "runtime/commit_manifest.json"))
...
def main():
    parser = argparse.ArgumentParser(description="发送成功后提交临时模板为正式模板。")
