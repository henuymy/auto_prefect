"""Build a reusable image/text message package from configured Excel workbooks."""
...
def get_output_paths(config, config_dir):
...
def optimize_png(image_path, capture):
    ...
    try:
        ...
        print("[WARN] 缺少 Pillow，跳过 PNG 优化: pip install pillow")
        return
    ...
    print(f"[INFO] PNG 优化完成: {original_size} -> {optimized_size} bytes")
...
def build_preview_image(package, preview_file):
    ...
    try:
        ...
        print("[WARN] 缺少 Pillow，跳过生成预览图: pip install pillow")
        return None
    ...
    try:
        ...
        print(f"[INFO] 预览图已写入: {preview_file}")
...
def build_message_package(config, config_path, visible=False):
    config_dir = config_path.parent
    image_dir, package_file, preview_file = get_output_paths(config, config_dir)
    default_capture = config.get("capture_defaults", {})
    workbooks = config.get("workbooks", [])
    if not workbooks:
        raise ValueError("config.json 中缺少 workbooks[]")
    ...
    try:
        for workbook_index, workbook_config in enumerate(workbooks, start=1):
            ...
            workbook_path = resolve_path(workbook_config["file"], config_dir)
            open_config = workbook_config.get("excel_open", {})
            print(f"[INFO] 打开 Excel: {workbook_path}")
            ...
            try:
                ...
                if open_config.get("refresh_before_capture", False):
                    ...
                    print(f"[INFO] 刷新查询和连接，超时 {timeout} 秒...")
                ...
                for report_index, report in enumerate(workbook_config.get("reports", []), start=1):
                    ...
                    for item_index, item in enumerate(report.get("items", []), start=1):
                        ...
                        if item_type == "image":
                            ...
                            print(f"[INFO] 已生成图片消息: {capture_result['path']}")
                        else:
                            ...
                            print(f"[INFO] 已生成文字消息: {workbook_name} / {report_name} / {sheet_name}")
    ...
    save_package(package_file, package)
    print(f"[INFO] 消息包已写入: {package_file}")
    return package, package_file


def main():
    parser = argparse.ArgumentParser(description="Build image/text message package from Excel workbooks.")
    parser.add_argument("--config", default=str(DEFAULT_CONFIG_PATH))
    parser.add_argument("--visible", action="store_true", help="Show Excel while processing.")
    args = parser.parse_args()

    config, config_path = load_config(args.config)
    package, _ = build_message_package(config, config_path, visible=args.visible)
    summary = {
        "generated_at": package["generated_at"],
        "item_count": len(package["items"]),
        "preview_image_file": package.get("preview_image_file"),
        "items": [
            {
                "type": item["type"],
                ...
                "image_path": item.get("capture", {}).get("path"),
            }
