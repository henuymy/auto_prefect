"""Download configured reports using staged cookies exported by autologin."""
...
def raise_for_status_with_context(response):
    ...
    try:
        ...
        print(f"[ERROR] HTTP {response.status_code}: {response.request.method} {response.url}")
        if response.history:
            print("[ERROR] Redirect history:")
            for item in response.history:
                print(f"  - {item.status_code} {item.request.method} {item.url} -> {item.headers.get('Location', '')}")
        allow_header = response.headers.get("Allow")
        if allow_header:
            print(f"[ERROR] Allow: {allow_header}")
        content_type = response.headers.get("Content-Type")
        if content_type:
            print(f"[ERROR] Content-Type: {content_type}")
        body_preview = response.text[:500].replace("\r", " ").replace("\n", " ")
        if body_preview:
            print(f"[ERROR] Body preview: {body_preview}")
        raise exc
...
def download_report(report, stage, output_dir, timeout, verify_ssl, trust_env, proxies):
...
def run(config_path, dry_run=False, debug=False):
    config_path = Path(config_path).resolve()
    config = load_json(config_path)
    cookie_dump_path = resolve_path(SKILL_DIR, config.get("cookie_dump_path", "../autologin/runtime/cookie_dump.json"))
    output_dir = resolve_path(SKILL_DIR, config.get("output_dir", "runtime/downloads"))
    manifest_path = resolve_path(SKILL_DIR, config.get("manifest_path", "runtime/download_manifest.json"))
    ...
    for report in reports:
        ...
        print(f"[INFO] 准备下载: {name} / stage={stage_name}")
        if debug:
            summary = summarize_request(report, stage)
            print(f"[DEBUG] Method: {summary['method']}")
            print(f"[DEBUG] URL: {summary['url']}")
            print(f"[DEBUG] Headers: {', '.join(summary['headers'])}")
            print(f"[DEBUG] Cookies: {', '.join(summary['cookies'])}")
            print(f"[DEBUG] Data keys: {', '.join(summary['data_keys'])}")
        if dry_run:
            results.append({
                "name": name,
                "stage": stage_name,
        ...
        results.append(download_report(report, stage, output_dir, timeout, verify_ssl, trust_env, report_proxies))
        print(f"[SUCCESS] 已下载: {results[-1]['output_path']}")
    ...
    print(f"[INFO] 下载清单: {manifest_path}")
    return manifest


def main():
    parser = argparse.ArgumentParser(description="使用 autologin Cookie JSON 下载配置报表。")
    parser.add_argument("--config", default=str(DEFAULT_CONFIG_PATH), help="配置文件路径")
    parser.add_argument("--dry-run", action="store_true", help="只检查配置和 Cookie stage，不发起下载")
    parser.add_argument("--debug", action="store_true", help="打印请求摘要，不打印敏感值")
    args = parser.parse_args()
    run(args.config, dry_run=args.dry_run, debug=args.debug)
