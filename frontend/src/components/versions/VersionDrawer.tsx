      ...
      <aside className="ml-auto flex h-full w-full max-w-2xl flex-col border-l border-border bg-background shadow-2xl">
        <div className="flex flex-col gap-3 border-b border-border p-4 sm:flex-row sm:items-center sm:justify-between sm:p-5">
          <div className="min-w-0">
            ...
            <div className="mt-1 break-all text-xs text-muted-foreground">{configName}</div>
          </div>
        ...
        <div className="min-h-0 flex-1 overflow-auto p-4 sm:p-5">
          <div className="space-y-2">
            {versions.map((version) => (
              <div key={version.id} className="grid gap-3 rounded-xl border border-border bg-card/70 p-3 sm:grid-cols-[1fr_120px_auto] sm:items-center">
                <div className="min-w-0">
